...
export const validateLeadTransition = (
  status: LeadStatus,
  closedReason?: string,
) => {
  if (status === "LOST" && !closedReason?.trim())
    return "closedReason is required for LOST leads.";
  return "";
};
...
export const validateQuoteTransition = (
  ...
  if (status === "REJECTED" && !reason?.trim())
    return "reason is required for REJECTED quotations.";
  return "";
};
...
export const validateBookingTransition = (
  ...
  if (status === "CANCELLED" && !cancellationReason?.trim())
    return "cancellationReason is required for CANCELLED bookings.";
  return "";
};

export const validateVisaTransition = (
  status: VisaStatus,
