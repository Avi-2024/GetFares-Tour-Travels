...
5. CREATE DESTINATION RECORD MODAL

```txt
    - season focus - it will be multi select from dropdown
    - category - it will be multi select too
```

6. VISA DESTINATIONS

```txt
    - add a field for 'price' it will be a number field and currency choosing option [<currency dropdown><amount field>]
```
