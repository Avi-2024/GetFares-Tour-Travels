-- =====================================================
-- MODULE 2 PRD HARDENING (Quotation Management)
...
ALTER TABLE quotations
  ADD COLUMN IF NOT EXISTS lead_to_quote_sent_minutes INT;

ALTER TABLE quotations
  ADD COLUMN IF NOT EXISTS response_category VARCHAR(30);

ALTER TABLE quotations
  ADD COLUMN IF NOT EXISTS response_sla_minutes INT;

ALTER TABLE quotations
  ADD COLUMN IF NOT EXISTS response_sla_breached BOOLEAN NOT NULL DEFAULT FALSE;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'chk_quotations_response_category'
  ) THEN
    ALTER TABLE quotations
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'chk_quotations_response_sla_minutes'
  ) THEN
    ALTER TABLE quotations
...
SET response_category = CASE
  ...
  WHEN template_snapshot->>'templateType' = 'READY_PACKAGE' THEN 'READY_PACKAGE'
  WHEN template_snapshot->>'templateType' = 'CUSTOM_ITINERARY' THEN 'CUSTOMIZED'
  ELSE 'CUSTOMIZED'
END
...
CREATE INDEX IF NOT EXISTS idx_quotations_response_category
  ON quotations(response_category);

CREATE INDEX IF NOT EXISTS idx_quotations_response_sla_breached
