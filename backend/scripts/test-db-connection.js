...
(async () => {
  ...
  try {
    ...
    const displayUrl = url.substring(0, 30) + '...' + url.substring(url.length - 15);
    ...
    if (typeof db.healthCheck === 'function') {
      ...
      if (health.adapter === 'postgres' && typeof db.query === 'function') {
        ...
        try {
          ...
          console.log(`   Destinations: ${result.rows[0]?.destinations || 0}\n`);
        } catch (queryErr) {
    ...
    } else if (err.code?.includes('PGERROR') || err.message?.includes('password')) {
      console.error('   → Check: Database credentials are incorrect');
    } else if (err.message?.includes('SSL')) {
      console.error('   → Check: SSL certificate issue - might need to update connection');
    }
