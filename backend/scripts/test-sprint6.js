...
async function main() {
  ...
  try {
    ...
    const updateCampaign = await request(
  ...
  try {
    ...
    addResult(
      "Customers business contract",
      true,
      `Customer created ${createdCustomerId}`,
    );
  } catch (error) {
  ...
  try {
    ...
    const addActivity = await request(
      `/api/complaints/${createdComplaintId}/activities`,
      {
        method: "POST",
        headers: authHeaders(adminToken),
        body: {
    ...
    const listActivities = await request(
      `/api/complaints/${createdComplaintId}/activities`,
      {
        headers: authHeaders(adminToken),
      },
    ...
    assert.ok(
      listActivities.json.data.some((item) => item.id === createdActivityId),
    );

    addResult(
      "Complaints workflow + activities",
      true,
      `Complaint ${createdComplaintId} with activity ${createdActivityId}`,
    );
  } catch (error) {
