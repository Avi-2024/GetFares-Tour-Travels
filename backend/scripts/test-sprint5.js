...
async function main() {
  ...
  try {
    ...
    assert.equal(
      withoutExchangeRate.json?.error?.code,
      "BOOKING_EXCHANGE_RATE_REQUIRED",
    );

    addResult(
      "Booking exchange guard",
      true,
      "Booking creation blocked without exchangeRate",
    );
  } catch (error) {
  ...
  try {
    ...
    const verifyDoc = await request(
      `/api/visa/documents/${documentId}/verify`,
      {
        method: "PATCH",
        headers: authHeaders(visaExecToken),
  ...
  try {
    ...
    addResult(
      ...
      true,
      "SUBMITTED -> APPROVED transition successful",
    );
  } catch (error) {
  ...
  try {
    ...
    addResult(
      "Visa summary report",
      true,
      `Total cases ${visaSummary.json?.data?.totalCases}`,
    );
  } catch (error) {
  ...
  try {
    ...
    addResult(
      "Management reports",
      true,
      "Lead/Revenue/Visa/Monthly summary reports returned 200",
    );
  } catch (error) {
