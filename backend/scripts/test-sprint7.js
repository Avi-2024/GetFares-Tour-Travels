...
async function main() {
  ...
  try {
    ...
    const markConverted = await request(
      `/api/leads/${leadThree.json?.data?.id}`,
      {
        method: "PATCH",
        headers: authHeaders(adminToken),
        body: {
    ...
    addResult(
      "Lead funnel seed",
      true,
      `Leads seeded (OPEN/LOST/CONVERTED with campaign ${campaignId})`,
    );
  } catch (error) {
    ...
    if (typeof container.db?.query === "function" && container.db.pool) {
      await container.db.query(
        ...
        [
          ...
          adminUserId,
          "CALL_OUTBOUND",
          "Call completed with customer",
        ],
      );
    }

    addResult(
      "Follow-up and call log seed",
      true,
      `Follow-ups created for lead ${leadOneId}`,
    );
  } catch (error) {
  ...
  try {
    ...
    addResult(
      "Revenue seed",
      true,
      `Quotation ${quotationId} and booking ${bookingId} created`,
    );
  } catch (error) {
  ...
  try {
    if (typeof container.db?.query === "function" && container.db.pool) {
      const supplierInsert = await container.db.query(
        ...
        `,
        [
          `Sprint7 Supplier ${seed}`,
          `s7-supplier-${seed}@example.com`,
          "9000000000",
        ],
      );
    ...
    addResult(
      "Supplier/Visa seed",
      true,
      `Visa created with supplier ${supplierId || "N/A"}`,
    );
  } catch (error) {
  ...
  try {
    ...
    addResult(
      ...
      true,
      "Sprint 7 report endpoints returned expected shapes",
    );
  } catch (error) {
