...
async function main() {
  ...
  try {
    ...
    addResult(
      "Auth register",
      true,
      "POST /api/auth/register returned 201 with token",
    );
  } catch (error) {
  ...
  try {
    ...
    addResult(
      "Auth login",
      true,
      "POST /api/auth/login returned 200 with token",
    );
  } catch (error) {
  ...
  try {
    ...
    addResult(
      "Lead list",
      true,
      `GET /api/leads returned ${preDuplicateCount} lead(s)`,
    );
  } catch (error) {
  ...
  if (!firstLeadId) {
    addResult(
      "Lead anti-duplication",
      false,
      "Cannot verify because lead creation failed",
    );
  } else {
    try {
      ...
      const postDuplicateCount = Array.isArray(listAfterDuplicate.json?.data)
        ? listAfterDuplicate.json.data.length
        : -1;

      const duplicatePrevented =
        duplicateLead.response.status === 409 ||
        postDuplicateCount === preDuplicateCount;
      ...
      addResult(
        "Lead anti-duplication",
        true,
        "Duplicate lead blocked or merged",
      );
    } catch (error) {
  ...
  for (const path of webhookPaths) {
    try {
      ...
      addResult(
        `Webhook ${path}`,
        true,
        `POST ${path} returned ${webhook.response.status}`,
      );
    } catch (error) {
