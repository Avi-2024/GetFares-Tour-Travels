function createVisaEvents({ eventBus, logger }) {
  return Object.freeze({
    ...
    emitDocumentAdded(payload) {
      logger.info(
        { visaCaseId: payload.visaCaseId, documentId: payload.id },
        "visa.document_added",
      );
    ...
    emitDocumentVerified(payload) {
      logger.info(
        {
          visaCaseId: payload.visaCaseId,
          documentId: payload.id,
          isVerified: payload.isVerified,
        },
        "visa.document_verified",
      );
      eventBus.emit("visa.document_verified", payload);
    },
    emitChecklistUpdated(payload) {
      logger.info(
        { bookingId: payload.bookingId, travelReady: payload.travelReady },
        "visa.checklist_updated",
      );
