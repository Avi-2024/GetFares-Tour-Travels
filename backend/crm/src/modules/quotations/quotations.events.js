function createQuotationsEvents({ eventBus, logger }) {
  return Object.freeze({
    ...
    emitSent(payload) {
      logger.info(
        { id: payload.id, sentBy: payload.sentBy },
        "quotations.sent",
      );
    ...
    emitViewed(payload) {
      logger.info(
        { id: payload.id, viewCount: payload.viewCount },
        "quotations.viewed",
      );
    ...
    emitPdfGenerated(payload) {
      logger.info(
        { id: payload.id, pdfUrl: payload.pdfUrl },
    ...
    emitMarginApproved(payload) {
      logger.info(
        { id: payload.id, approvedBy: payload.approvedBy },
