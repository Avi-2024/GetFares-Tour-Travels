...
const PACKAGE_STATUSES: PackageStatus[] = ["DRAFT", "ACTIVE", "EXPIRED", "SOLD_OUT"];
...
const PackagesPage: React.FC = () => {
  ...
  }, [categoryFilter, destinationFilter, packagesService, search, selectedId, statusFilter]);
  ...
  const loadEnquiries = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        setEnquiryError(getApiErrorMessage(err, "Failed to load package enquiries."));
        setEnquiries([]);
      } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage package pricing, validity, category, status, and website publish policy.
          </p>
        </div>
        <button
          onClick={handleNew}
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <div className="mt-2 text-xs text-gray-600 dark:text-gray-300">
                    Price {item.startingPrice.toLocaleString()} • Markup {item.markupPercent}%
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <input
                ...
                onChange={(event) =>
                  setForm((prev) => ({ ...prev, markupPercent: event.target.value }))
                }
              />
              <input
                className="field-input"
            ...
            <label className="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
              <input
                ...
                onChange={(event) =>
                  setForm((prev) => ({ ...prev, publishToWebsite: event.target.checked }))
                }
              />
              Publish to website
            </label>
            <label className="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
              <input
                ...
                onChange={(event) =>
                  setForm((prev) => ({ ...prev, isSoldOut: event.target.checked }))
                }
              />
              Mark sold out
            </label>
      ...
      <SurfaceCard>
        ...
        {!selectedId ? (
          <p className="mt-3 text-sm text-gray-500">Select a package to manage enquiries.</p>
        ) : (
          <>
            <div className="mt-4 grid grid-cols-1 gap-2 md:grid-cols-4">
              <input
                ...
                onChange={(event) =>
                  setEnquiryDraft((prev) => ({ ...prev, leadId: event.target.value }))
                }
              />
              <input
                className="field-input"
                ...
                onChange={(event) =>
                  setEnquiryDraft((prev) => ({ ...prev, fullName: event.target.value }))
                }
              />
              <input
                className="field-input"
                ...
                onChange={(event) =>
                  setEnquiryDraft((prev) => ({ ...prev, phone: event.target.value }))
                }
              />
              <input
                className="field-input"
                ...
                onChange={(event) =>
                  setEnquiryDraft((prev) => ({ ...prev, email: event.target.value }))
                }
              />
              <input
                type="date"
                ...
                onChange={(event) =>
                  setEnquiryDraft((prev) => ({ ...prev, travelDate: event.target.value }))
                }
              />
              <input
                className="field-input"
              ...
              <input
                ...
                onChange={(event) =>
                  setEnquiryDraft((prev) => ({ ...prev, source: event.target.value }))
                }
              />
              <button
            ...
            <div className="mt-4 space-y-2">
              {enquiryLoading ? (
                ...
                <p className="text-sm text-gray-500">No enquiries for this package yet.</p>
              ) : (
