...
const dueLabel = (payable: SupplierPayable) => {
  ...
  if (payable.dueInDays < 0) return `Overdue by ${Math.abs(payable.dueInDays)} day(s)`;
...
const SuppliersPage: React.FC = () => {
  ...
  const payableStats = useMemo(() => {
    const totalPayable = payables.reduce((sum, row) => sum + row.payableAmount, 0);
  ...
  const handleRunDeadlineAlerts = async () => {
    ...
    try {
      ...
      const summary = unwrapObject(response) as { processed?: number; triggered?: number };
      setNotice(
        `Deadline alerts checked ${toNumber(summary.processed, 0)} payables and triggered ${toNumber(summary.triggered, 0)} alert(s).`,
      );
      if (selectedSupplierId) {
        await loadPayables(selectedSupplierId);
      }
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Clear workflow for supplier profile, contract validity, and payable deadlines.
          </p>
        </div>
        <button
      ...
      <SurfaceCard className="p-4">
        ...
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
          1) Select or create supplier. 2) Keep contract and rate validity updated.
          3) Add payables with due dates. 4) Track due risk and run alerts when needed.
        </p>
      </SurfaceCard>

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        <Metric title="Total" value={String(supplierStats.total)} tone="slate" />
        <Metric title="Active" value={String(supplierStats.active)} tone="green" />
        <Metric title="Inactive" value={String(supplierStats.inactive)} tone="slate" />
        <Metric title="Due Soon" value={String(payableStats.dueSoon)} tone="amber" />
        <Metric title="Overdue" value={String(payableStats.overdue)} tone="red" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[340px_1fr]">
        <SurfaceCard className="overflow-hidden p-0">
          ...
          {loadingSuppliers ? (
            <div className="p-4 text-sm text-gray-500">Loading suppliers...</div>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {supplier.country || "No country"} | {supplier.supplierCurrency || "INR"}
                      </p>
                    </div>
                    <button
                      onClick={(event) => {
        ...
        <div className="space-y-5">
            ...
            {!selectedSupplier ? (
              ...
              <div className="mt-3 space-y-3">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
                  <Info label="Country" value={selectedSupplier.country || "Not set"} />
                  <Info label="Currency" value={selectedSupplier.supplierCurrency || "INR"} />
                  <Info label="Rate Valid" value={formatDate(selectedSupplier.rateValidUntil)} />
                  <Info
                    label="Pay Deadline"
                    value={formatDate(selectedSupplier.paymentDeadlineDate)}
                  />
                  <Info label="Email" value={selectedSupplier.email || "Not set"} />
                  <Info label="Phone" value={selectedSupplier.phone || "Not set"} />
                </div>
                <Info label="Contract URL" value={selectedSupplier.contractUrl || "Not set"} />
                <Info
          ...
          <SurfaceCard className="p-4">
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
              <Info label="Total Payable" value={payableStats.totalPayable.toLocaleString()} />
              <Info label="Total Paid" value={payableStats.totalPaid.toLocaleString()} />
              <Info label="Pending" value={payableStats.pending.toLocaleString()} />
            </div>
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
              <input
                ...
                onChange={(event) =>
                  setPayableForm((prev) => ({ ...prev, bookingId: event.target.value }))
                }
              ...
              <input
                ...
                onChange={(event) =>
                  setPayableForm((prev) => ({ ...prev, paidAmount: event.target.value }))
                }
                className="field-input"
                ...
                filteredPayables.map((payable) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span
                        className={`rounded-full border px-2 py-0.5 text-[11px] font-medium ${dueBadgeClass(payable)}`}
                      >
                        {dueLabel(payable)}
                      </span>
                    </div>
                    <div className="mt-2 flex items-center justify-between text-xs text-gray-600 dark:text-gray-300">
                      <span>Payable: {payable.payableAmount.toLocaleString()}</span>
          ...
          <SurfaceCard className="p-4">
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <input
                ...
                onChange={(event) =>
                  setSupplierForm((prev) => ({ ...prev, country: event.target.value }))
                }
                className="field-input"
                placeholder="Country"
              />
            ...
            <div className="mt-3 flex items-center gap-2">
              <input
                ...
                onChange={(event) =>
                  setSupplierForm((prev) => ({ ...prev, isActive: event.target.checked }))
                }
              />
              <label
