...
const Settings: React.FC = () => {
  ...
  const [roleCountryFilter, setRoleCountryFilter] =
  ...
  useEffect(() => {
    ...
    const exists = roles.some(
      (role) => role.id === selectedRolePermissionsRoleId,
    );
    if (!exists) {
            ...
            <div className="mt-4 space-y-4">
              ...
              <div>
                ...
                <SearchableDropdown
                  ...
                  options={ROLE_COUNTRY_OPTIONS}
                  onChange={(value) =>
                    setCreateRoleCountry(value as CountryCode)
                  }
...
const PermissionsMultiSelect: React.FC<PermissionsMultiSelectProps> = ({
  ...
  const filteredOptions = useMemo(() => {
    ...
    return options.filter((option) => {
      const haystack =
        `${option.key} ${option.description ?? ""}`.toLowerCase();
